...
class ModelsManager(FinalsManager):
    ...
    #     results = await loop.run_in_executor(_executor, requests.post(
    #         f"http://{self.local_ip}:5004/identify",
    #         json={"instances": [{"b64": image_str, "caption": caption}]},
    #     ))
    #     return results.json()["predictions"][0]

    # async version of run_vlm to fix the issue of the synchronous request call blocking up the ping-pong keepalive and heartbeat mechanism (which causes unexpected websockets diconnections).
